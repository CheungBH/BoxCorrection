...
device = opt.device
model_path = ""
logger = ErrorAnalysisLogger("/".join(model_path.split("/")[:-1]))
...
for i, data in enumerate(loader):
    ...
    errors.append([image_name[0], fg, in_cls_loss.tolist(), in_reg_loss.tolist(), out_cls_loss.tolist(), out_reg_loss.tolist()])
